...
출력 (candidate dict 추가):
  {
    "buy_questions": {
      ...
      "catalyst": "입력된 촉매 데이터 기반 catalyst",
...

{
  ...
  "catalyst": "입력된 촉매 데이터가 있으면 그 fact 기반 catalyst. 없으면 '촉매 데이터 미연결'",
...
def _is_missing_catalyst(text: str) -> bool:
    """촉매 데이터가 실제 입력되지 않았을 때 반복 문구를 숨기기 위한 판정."""
...
def _build_user_prompt(candidate: dict, fundamental: dict, today: str) -> str:
    ...
    catalyst_context = candidate.get("catalyst_context", {}) or {}
    catalyst_status = catalyst_context.get("status", "")
    catalyst_news = catalyst_context.get("news", []) or []
    catalyst_hits = catalyst_context.get("positive_hits", []) or []
    catalyst_risks = catalyst_context.get("risk_hits", []) or []
    if catalyst_status == "found" and catalyst_news:
        facts.append("촉매 데이터: 최근 뉴스/이벤트 확인")
        for n in catalyst_news[:3]:
            headline = str(n.get("headline", "") or "").strip()
            source = str(n.get("source", "") or "").strip()
            if headline:
                facts.append(f"- {headline}" + (f" ({source})" if source else ""))
        if catalyst_hits:
            facts.append(f"촉매 키워드: {catalyst_hits[:2]}")
    elif catalyst_status == "risk" and catalyst_risks:
        facts.append("촉매 데이터: 리스크 뉴스 확인")
        for hit in catalyst_risks[:2]:
...
def answer_buy_questions(
    ...
    for key in required_keys:
        if key not in result:
            result[key] = "정보 부족"
    if _is_missing_catalyst(str(result.get("catalyst", ""))):
        result["catalyst"] = "촉매 데이터 미연결"
...
def format_buy_questions_telegram(candidate: dict) -> str:
    ...
    if catalyst and not _is_missing_catalyst(catalyst):
