...
def scout_candidates_to_m6_entries(candidates: list[dict]) -> list[dict]:
    ...
    for c in candidates:
        ...
        entries.append({
            ...
            "catalyst_classification": catalyst_context.get("classification", ""),
            "catalyst_freshness": (catalyst_context.get("freshness") or {}).get("status", ""),
            "catalyst_reaction": (catalyst_context.get("price_volume_reaction") or {}).get("status", ""),
            "catalyst_top3_excluded_reason": catalyst_context.get("top3_excluded_reason", ""),
...
def _compute_performance(history: list[dict]) -> list[dict]:
    ...
    for h in history:
        ...
        results.append({
            ...
            "catalyst_classification": h.get("catalyst_classification", ""),
            "catalyst_freshness": h.get("catalyst_freshness", ""),
            "catalyst_reaction": h.get("catalyst_reaction", ""),
            "catalyst_top3_excluded_reason": h.get("catalyst_top3_excluded_reason", ""),
