...
def fetch_market_reaction() -> dict[str, dict]:
    ...
    """
    result = {}
    for label, symbol in MARKET_REACTION_SYMBOLS.items():
...
def get_yesterday_announced_events() -> list[dict]:
    ...
    fred_requested = len((_load_settings()["regime"]["macro_calendar"].get("fred_series") or []))
    source_coverage = {
        "status": "OK" if len(macro_data) == fred_requested and len(market_data) == len(MARKET_REACTION_SYMBOLS) else "DEGRADED",
        "fred_requested": int(fred_requested),
        "fred_collected": int(len(macro_data)),
        "market_requested": int(len(MARKET_REACTION_SYMBOLS)),
        "market_collected": int(len(market_data)),
    }
    ...
    for evt in events:
        ...
        evt_copy["source_coverage"] = source_coverage
