"""
src/collectors/global_ohlcv.py — 4개국 일봉 OHLCV 통합 fetcher

역할:
  - 종목 리스트 받아서 일봉 OHLCV 일괄 수집
  - 국가별 적합한 데이터 소스 자동 선택
  - 캐시 (당일 1회만 fetch)
  - SCOUT의 Stage 3 정밀 신호 계산용

데이터 소스 매핑:
  US     → yfinance batch download (가장 안정적)
  KR     → pykrx
  JP     → yfinance (.T 접미사)
  CN_ADR → yfinance (미국 거래소 종목)
